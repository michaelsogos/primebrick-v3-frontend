...
`/api/v1/auth/login` and reacts to the backend auth config. The
username/password fields are always rendered (form auth is an invariant);
when `enable_webauthn` is on (and the browser supports WebAuthn) it renders a
...
- **Auth-config driven**: reads `authConfigState` (`enable_webauthn`).
  The password form is always rendered (form auth is an invariant); the
  passkey button only renders when WebAuthn is enabled and the browser
  supports it.
