<script lang="ts">
  ...
  let rows = $state<OrganizationListRow[]>([]);
  ...
  const storageKeyPrefix = 'pb:organizations:list:';
  ...
  const metaCacheKey = '__pbOrganizationMetaCache';
  const metaInFlightKey = '__pbOrganizationMetaInFlight';

  function getMetaCache(): OrganizationMeta | null {
    return ((globalThis as any)[metaCacheKey] ?? null) as OrganizationMeta | null;
  }
  function getMetaInFlight(): Promise<OrganizationMeta> | null {
    return ((globalThis as any)[metaInFlightKey] ?? null) as Promise<OrganizationMeta> | null;
  }
  function setMetaCache(next: OrganizationMeta | null) {
    (globalThis as any)[metaCacheKey] = next;
  }
  function setMetaInFlight(next: Promise<OrganizationMeta> | null) {
  ...
  async function loadMeta() {
    ...
    setMetaInFlight(
      (async () => {
        const metaRes = await apiFetchWithTimeout('/api/v1/entities/organization/meta', undefined, 30_000);
        if (!metaRes.ok) {
        ...
        const next = (await metaRes.json()) as OrganizationMeta;
        setMetaCache(next);
        return next;
      })()
    );
  ...
  async function loadRows() {
    ...
    const listRes = await apiFetchWithTimeout(
      `/api/v1/entities/organization/list?${qs.toString()}`,
  ...
  async function refreshRows(opts?: { clampPage?: boolean }) {
    ...
    try {
      ...
      const tags: AppErrorTag[] = [
        ...
        ...(status !== null ? [{ label: `HTTP ${status}`, tone: toneForImpact } as const] : []),
      ];
  ...
  async function bootstrapOrganizationsList() {
    loading = true;
    error = null;
  ...
  $effect(() => {
    if (didInit) return;
    didInit = true;
    void bootstrapOrganizationsList();
  });
    ...
    return onConnectivityRestored(() => {
      void (async () => {
        if (meta) {
          ...
          await bootstrapOrganizationsList();
        }
      })();
    });
  });
  ...
  function organizationSearchShouldQuery(trimmed: string): boolean {
  ...
  function onSearchInput(v: string) {
    ...
    const shouldSearch = organizationSearchShouldQuery(trimmed);
