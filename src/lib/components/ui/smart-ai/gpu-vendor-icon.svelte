<script lang="ts">
  /**
   * GpuVendorIcon — brand icon for a detected GPU vendor.
   *
   * Vendors: nvidia | amd | intel | apple | generic (iGPU / unknown).
   * NVIDIA/AMD/Apple use Bootstrap brand icons; Intel uses the simple-icons
   * wordmark path; anything else falls back to a generic GPU card glyph.
   */
  import BiNvidia from '~icons/bi/nvidia';
  import BiAmd from '~icons/bi/amd';
  import BiApple from '~icons/bi/apple';
  import BiGpuCard from '~icons/bi/gpu-card';

  let { vendor, class: className = 'size-4' }: {
    vendor: string | null;
    class?: string;
  } = $props();
</script>
...
{#if vendor === 'nvidia'}
  <BiNvidia class={className} style="color:#76B900" />
{:else if vendor === 'amd'}
  <BiAmd class={className} style="color:#ED1C24" />
{:else if vendor === 'apple'}
  <BiApple class={className} />
{:else if vendor === 'intel'}
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class={className} style="color:#0068B5" fill="currentColor" aria-hidden="true">
